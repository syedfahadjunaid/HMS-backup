...
export default function OtherServiceChargesTable({
  ...
  React.useEffect(() => {
    setItemData(allOtherServiceCharges);
  }, []);

  React.useEffect(() => {
    setAllOtherServiceCharges([...itemData]);
  }, [itemData]);
  ...
  const mappedItemData = allOtherServiceCharges?.map((item, index) => {
    return {
      itemId: index + 1,
  ...
  React.useEffect(() => {
    ...
    setOtherServicesChargesItemSubTotal(subTotal);
  }, [mappedItemData]);
    ...
    <div className='flex flex-col gap-[1rem]'>
      ...
      <div className='flex flex-row w-full justify-end'>
        <div className='flex flex-row gap-[2rem] font-[600] px-[2rem]'>
          <h3>Sub Total:</h3>
          <h3>{`₹ ${otherServicesChargesSubTotal.toFixed(2)}`}</h3>
        </div>
      </div>
      {/* Modal For Item */}
      <Modal

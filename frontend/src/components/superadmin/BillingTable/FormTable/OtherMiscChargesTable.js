...
export default function OtherMiscChargesTable({
  allOtherMiscCharges,
  setAllOtherMiscCharges,
  otherMiscChargesSubTotal,
  setOtherMiscChargesItemSubTotal,
}) {
  // Add Item In Billing--------------------------
  ...
  React.useEffect(() => {
    setItemData(allOtherMiscCharges);
  }, []);
  React.useEffect(() => {
    setAllOtherMiscCharges([...itemData]);
  }, [itemData]);
  ...
  const mappedItemData = allOtherMiscCharges?.map((item, index) => {
    return {
      itemId: index + 1,
  ...
  React.useEffect(() => {
    ...
    setOtherMiscChargesItemSubTotal(subTotal);
  }, [mappedItemData]);
    ...
    <div className='flex flex-col gap-[1rem]'>
      ...
      <div className='flex flex-row w-full justify-end'>
        <div className='flex flex-row gap-[2rem] font-[600] px-[2rem]'>
          <h3>Sub Total:</h3>
          <h3>{`₹ ${otherMiscChargesSubTotal.toFixed(2)}`}</h3>
        </div>
      </div>
      {/* Modal For Item */}
      <Modal

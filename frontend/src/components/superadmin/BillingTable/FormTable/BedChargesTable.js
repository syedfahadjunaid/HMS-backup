...
export default function BedChargesTable({
  allBedChargesItems,
  setAllBedChargesItems,
  bedChargesSubTotal,
  setBedChargesSubTotal,
}) {
  // Add Item In Billing--------------------------
  ...
  React.useEffect(() => {
    setItemData(allBedChargesItems);
  }, []);

  React.useEffect(() => {
    setAllBedChargesItems([...itemData]);
  }, [itemData]);
  ...
  const mappedItemData = allBedChargesItems?.map((item, index) => {
    return {
      itemId: index + 1,
  ...
  // const [bedChargesSubTotal, setBedChargesSubTotal] = useState(0);

  useEffect(() => {
    const subTotal = allBedChargesItems?.reduce(
      (accumulator, currentValue) => accumulator + currentValue.itemTotalAmount,
      0
    );
    setBedChargesSubTotal(subTotal);
  }, [allBedChargesItems]);
    ...
    <div className='flex flex-col gap-[1rem]'>
      ...
      <div className='flex flex-row w-full justify-end'>
        <div className='flex flex-row gap-[2rem] font-[600] px-[2rem]'>
          <h3>Sub Total:</h3>
          <h3>{`₹ ${bedChargesSubTotal.toFixed(2)}`}</h3>
        </div>
      </div>
      {/* Modal For Item */}
      <Modal

...
export default function BasicHospitalChargesTable({
  ...
  React.useEffect(() => {
    setItemData(allBasicHospitalCharges);
  }, []);

  React.useEffect(() => {
    setAllBasicHospitalCharges([...itemData]);
  }, [itemData]);
  ...
  const mappedItemData = allBasicHospitalCharges?.map((item, index) => {
    return {
      itemId: index + 1,
  ...
  React.useEffect(() => {
    ...
    setBasicHospitalChargesItemSubTotal(subTotal);
  }, [mappedItemData]);
    ...
    <div className='flex flex-col gap-[1rem]'>
      ...
      <div className='flex flex-row w-full justify-end'>
        <div className='flex flex-row gap-[2rem] font-[600] px-[2rem]'>
          <h3>Sub Total:</h3>
          <h3>{`₹ ${basicHospitalChargesSubTotal.toFixed(2)}`}</h3>
        </div>
      </div>
      {/* Modal For Item */}
      <Modal

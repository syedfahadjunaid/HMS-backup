...
export default function MedicinesTable({
  ...
  setMedicinesChargesItemSubTotal,
}) {
  // Add Item In Billing--------------------------
  ...
  const mappedItemData = allMedicines?.map((item, index) => {
    return {
      itemId: index + 1,
  ...
  React.useEffect(() => {
    const subTotal = allMedicines?.reduce(
      (accumulator, currentValue) => accumulator + currentValue.itemTotalAmount,
      0
    );
    setMedicinesChargesItemSubTotal(subTotal);
  }, [mappedItemData]);
    ...
    <div className='flex flex-col gap-[1rem]'>
      ...
      <div className='flex flex-row w-full justify-end'>
        <div className='flex flex-row gap-[2rem] font-[600] px-[2rem]'>
          <h3>Sub Total:</h3>
          <h3>{`₹ ${medicinesChargesSubTotal.toFixed(2)}`}</h3>
        </div>
      </div>
      {/* Modal For Item */}
      <Modal

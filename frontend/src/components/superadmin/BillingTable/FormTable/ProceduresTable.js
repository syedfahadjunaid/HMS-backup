...
export default function ProceduresTable({
  ...
  setProceduresChargesItemSubTotal,
}) {
  // Add Item In Billing--------------------------
  ...
  React.useEffect(() => {
    setItemData(allProcedures);
  }, []);

  React.useEffect(() => {
    setAllProcedures([...itemData]);
  }, [itemData]);
  ...
  const mappedItemData = allProcedures?.map((item, index) => {
    return {
      itemId: index + 1,
  ...
  React.useEffect(() => {
    ...
    setProceduresChargesItemSubTotal(subTotal);
  }, [mappedItemData]);
    ...
    <div className='flex flex-col gap-[1rem]'>
      ...
      <div className='flex flex-row w-full justify-end'>
        <div className='flex flex-row gap-[2rem] font-[600] px-[2rem]'>
          <h3>Sub Total:</h3>
          <h3>{`₹ ${proceduresChargesSubTotal.toFixed(2)}`}</h3>
        </div>
      </div>
      {/* Modal For Item */}
      <Modal

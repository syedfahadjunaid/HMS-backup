...
export default function DoctorVisitChargesTable({
  ...
  const [itemQuantity, setItemQuanity] = React.useState(1);
  ...
  useEffect(() => {
    const doctorData = doctors?.find(
      (doctor) => doctorId.value === doctor.doctorId
    );
    const doctorProDetails = doctorProfessionalDetails?.find(
      (doctor) => doctorId.value === doctor.doctorId
    );
    setItemName(doctorData?.doctorName);
    setItemPrice(Number(doctorProDetails?.doctorFee));
    setItemQuanity(1);
  }, [doctorId]);

  React.useEffect(() => {
    setItemData(allDoctorVisitCharges);
  }, []);

  React.useEffect(() => {
    setAllDoctorVisitCharges([...itemData]);
  }, [itemData]);
  ...
  const mappedItemData = allDoctorVisitCharges?.map((item, index) => {
    return {
      itemId: index + 1,
  ...
  React.useEffect(() => {
    const subTotal = mappedItemData?.reduce(
      (accumulator, currentValue) => accumulator + currentValue.itemPrice,
      0
    );
    setDoctorVisitChargesItemSubTotal(subTotal);
  }, [mappedItemData]);
  ...
  const configForITEM = [
    ...
    // {
    //   label: "Amount",
    //   render: (list) => `₹ ${list.itemTotalAmount}`,
    // },
    {
      label: "Action",
      render: (list) => (
  ...
  const renderedDoctorForDropdownBilling = doctors?.map((data) => {
    return {
      value: data.doctorId,
    ...
    <form className='' onSubmit={handleItemSubmit}>
      <div className='grid grid-cols-2 gap-[2rem] pb-[2rem]'>
        <div className='flex flex-col gap-[6px]'>
          <label className='text-[14px]'>Doctor Reg Id *</label>
          <Select
            className='text-[12px]'
            options={renderedDoctorForDropdownBilling}
        ...
        <div className='flex flex-col gap-[6px]'>
          <label className='text-[14px]'>Fees *</label>
          <input
            className='py-[10px] outline-none border-b'
            type='number'
    ...
    <div className='flex flex-col gap-[1rem]'>
      ...
      <div className='flex flex-row w-full justify-end'>
        <div className='flex flex-row gap-[2rem] font-[600] px-[2rem]'>
          <h3>Sub Total:</h3>
          <h3>{`₹ ${doctorVisitChargesSubTotal.toFixed(2)}`}</h3>
        </div>
      </div>
      {/* Modal For Item */}
      <Modal

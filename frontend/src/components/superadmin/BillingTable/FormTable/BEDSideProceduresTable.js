...
export default function BEDSideProceduresTable({
  ...
  React.useEffect(() => {
    setItemData(allBEDSideProcedures);
  }, []);

  React.useEffect(() => {
    setAllBEDSideProcedures([...itemData]);
  }, [itemData]);
  ...
  const mappedItemData = allBEDSideProcedures?.map((item, index) => {
    return {
      itemId: index + 1,
  ...
  useEffect(() => {
    const subTotal = allBEDSideProcedures?.reduce(
      (accumulator, currentValue) => accumulator + currentValue.itemTotalAmount,
      0
    );
    setBedSideProceduresChargesSubTotal(subTotal);
  }, [allBEDSideProcedures]);
    ...
    <div className='flex flex-col gap-[1rem]'>
      ...
      <div className='flex flex-row w-full justify-end'>
        <div className='flex flex-row gap-[2rem] font-[600] px-[2rem]'>
          <h3>Sub Total:</h3>
          <h3>{`₹ ${bedSideProceduresChargesSubTotal.toFixed(2)}`}</h3>
        </div>
      </div>
      {/* Modal For Item */}
      <Modal
